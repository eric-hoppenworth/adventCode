...
fs.readFile("./puzzle.txt","utf8",function(err,data){
    ...
    // const testCommands = "104,1125899906842624,99".split(',').map(a => parseInt(a, 10));
    // let computer = new IntComputer(commands, [1]);
    let computer = new IntComputer(commands, [2], "NON_RECURSIVE");

    while(true) {
        let result = computer.handleOpcode();
        if (result === 1) {
            break;
        }
    }
